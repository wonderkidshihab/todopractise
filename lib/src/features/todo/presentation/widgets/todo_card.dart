...
class TodoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        Navigator.pushNamed(context, RoutesName.todoDetails, arguments: todo.id);
      },
      child: Card(
        child: Dismissible(
          key: Key(todo.id),
          ...
          child: ListTile(
            title: Text(todo.title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            subtitle: Text(
              todo.description,
              maxLines: 2,
